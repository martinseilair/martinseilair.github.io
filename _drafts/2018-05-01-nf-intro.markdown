...
Now that we have a good idea how the model will behave, you have the choice of either going directly to the first post about the discrete Bayes filter or to learn more about the model of the dynamic \\(p(x_{t+1}\|x_{t}, u_{t})\\) and observation \\(p(y_t\|x_t)\\) of the example in the following section. 
...
depends on the distance as well. The farer we are away from the tree, the more noise will be present in the signal.

By moving with your mouse or finger over the race track you will notice two things. Again there appears a strip, this time inside the race track and in a brownish color. Furthermore, you will notice another light green strip at the trunk of the tree. Both strips are showing parts of our observation model \\(p(y_t\|x_t)\\). The light green strip shows the probability of which distance measurement is observed at the current state. We notice, that the maximum probability density is at the true distance. The variance is varying depending on the distance to the tree. If the we have the distance \\(d\\) to our currenThe other strip shows the function \\(p(y_t=d\|x_t)\\). Please be aware, that this is not a probability density, because it is not normalized.
...
Now that we know everything about our model we are well prepared to start the series with the discrete bayes filter.
