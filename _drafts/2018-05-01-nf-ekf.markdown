...
<script>
	...
	n_scene.loaded = function(){
		...
		this.bf = init_bayes_filter(this.rc, this.rt);
		...
		this.rt.init_strip("outer", normalize_vector(this.bf.posterior), outer_color, 60);

		this.ekf = init_ekf_1D(this.rc, this.rc.state, 2*this.rc.sigma_o_no_cache(this.rc.state));
		...
		this.restart = function(){
			...
			this.bf.reset();
			this.ekf.reset(this.rc.state, 2*this.rc.sigma_o_no_cache(this.rc.state))
			this.rt.hide_strip("inner");
	...
	n_scene.step = function(){
		...
		if(this.t % 4 == 0){
			...
			// update posterior strip ekf
			document.getElementById("race_track_mar_loc_likelihood").style.display="block";
		}else if(this.t % 4 == 2){
			this.rt.show_strip("inner");
			// update likelihood strip ekf
